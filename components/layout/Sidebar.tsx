...
export function Sidebar() {
  ...
  const { sidebarOpen, setSidebarOpen, setShowSettings, setShowMemories, setShowIntegrations, showToast, convSearch, setConvSearch } = useUIStore();
  ...
  return (
    <aside style={{
      width: 260, flexShrink: 0, background: "rgba(2,2,8,.97)", borderRight: "1px solid var(--border-glow)",
      display: "flex", flexDirection: "column", padding: "16px 12px", gap: 12, overflow: "hidden",
      ...
      <button onClick={() => { newConversation(); showToast("Nova conversa ✓", "info"); setSidebarOpen(false); }}
        style={{ background: "transparent", border: "1px solid var(--border-active)", color: "var(--neon-cyan)", fontFamily: "Orbitron,sans-serif", fontSize: 11, padding: 10, borderRadius: 8, cursor: "pointer", letterSpacing: "0.1em", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "all .2s" }}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontFamily: "JetBrains Mono,monospace", fontSize: 9, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.2em" }}>HISTÓRICO</span>
        {conversations.length > 0 && (
          <span onClick={() => { if (window.confirm("Apagar TODO o histórico?")) { conversations.forEach(c => deleteConversation(c.id)); newConversation(); showToast("Histórico apagado", "error"); } }}
      ...
      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 3 }}>
        {sorted.map(conv => (
          <div key={conv.id} style={{ padding: "8px 10px", borderRadius: 8, cursor: "pointer", border: `1px solid ${conv.id === activeConvId ? "var(--border-glow)" : "transparent"}`, background: conv.id === activeConvId ? "rgba(0,245,255,.06)" : undefined, display: "flex", alignItems: "center", gap: 4, position: "relative", transition: "all .2s" }}
      ...
      <nav style={{ display: "flex", flexDirection: "column", gap: 4, borderTop: "1px solid var(--border-glow)", paddingTop: 12 }}>
        {[
          { icon: "⚙", label: "Configurações", action: () => setShowSettings(true) },
          { icon: "🔌", label: "Integrações", action: () => setShowIntegrations(true) },
          { icon: "🧠", label: `Memórias ${memories.length > 0 ? `(${memories.length})` : ""}`, action: () => setShowMemories(true) },
        ].map(item => (
          <div key={item.label} onClick={item.action}
            style={{ display: "flex", alignItems: "center", gap: 10, padding: 10, borderRadius: 8, cursor: "pointer", fontSize: 12, color: "var(--text-secondary)", transition: "all .2s", border: "1px solid transparent" }}
