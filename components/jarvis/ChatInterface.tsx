...
// Normaliza formato Groq (OpenAI-compatible) e Anthropic para formato interno
function normalizeChunk(raw: unknown): { type: string; content: string } | null {
  ...
  if (Array.isArray(r.choices) && r.choices.length > 0) {
    ...
    if (firstChoice && typeof firstChoice === "object") {
      ...
      if (delta && typeof delta === "object" && typeof (delta as Record<string, unknown>).content === "string") {
        return { type: "response", content: (delta as Record<string, unknown>).content as string };
      }
    }
  }
  ...
  if (r.type === "content_block_delta") {
    ...
    if (delta && typeof delta === "object" && typeof (delta as Record<string, unknown>).text === "string") {
      return { type: "response", content: (delta as Record<string, unknown>).text as string };
    }
  }

  // Formato interno já normalizado: { type: "response", content: "..." }
  if (typeof r.type === "string" && typeof r.content === "string") {
...
📎 ${attachments.map(a => a.type === "link" ? a.url : a.name).join(", ")}` : ""), created_at: new Date().toISOString() };
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          if (payload === "[DONE]") {
            ...
            try { rawEvent = JSON.parse(payload); } catch { continue; }
            const normalized = normalizeChunk(rawEvent);
            event = (normalized ?? rawEvent) as SSEEvent;
          }
      ...
      if (sseBuffer.trim().startsWith("data: ")) {
        const payload = sseBuffer.trim().slice(6).trim();
        if (payload === "[DONE]") {
          finishResponse();
        }
      }
